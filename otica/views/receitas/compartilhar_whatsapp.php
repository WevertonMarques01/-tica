...
try {
    ...
    $mensagem = "🔬 *RECETA ÓPTICA*\n\n";
    $mensagem .= "👤 *Cliente:* {$receita['cliente_nome']}\n";
    $mensagem .= "📅 *Data:* " . ($receita['data_receita'] ? date('d/m/Y', strtotime($receita['data_receita'])) : '-') . "\n\n";
    
    $mensagem .= "👁️ *OLHO DIREITO (OD):*\n";
    ...
    if (!empty($receita['adicao_od'])) $mensagem .= "  Adição: {$receita['adicao_od']}\n";
    ...
    if (!empty($receita['adicao_oe'])) $mensagem .= "  Adição: {$receita['adicao_oe']}\n";
